...
function ChatBot() {
  ...
  const chatContext =
    ...
    - Surgical team: Can see the patient status board and update the patient status but cannot add patients or update patient information.
    ...
    When asked about a patient, you can only provide information about status and name if the user role is a guest and surgical team. You can provide all information if the user is admin.`;
  ...
  return (
    <>
      <div className="fixed bottom-4 right-4 ml-4 flex flex-col items-end max-w-xl text-white gap-3">
        {openChat && (
          <div className="p-4 border flex flex-col w-full justify-right items-right backdrop-blur-2xl bg-white/30 border-gray-300/50 shadow-md rounded-4xl gap-3">
            <div>
              <ul
                ref={chatRef}
                id="chat-log"
              ...
              >
                {chatLog.map((entry, index) => (
                  <li
                    key={index}
                    className={`mb-2 rounded-2xl px-5 py-2 flex flex-col shadow-lg ${
                      entry.role === "user"
            ...
            <div className="border-gray-300 bg-gray-600/50 rounded-full flex px-1 items-center shadow-lg ">
              <input
                className="border p-2 w-full border-none focus:outline-none text-wrap overflow-y-auto resize-none max-h-[8rem] "
                type="text"
